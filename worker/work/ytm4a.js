...
module.exports = {
 ...
 try {
 let ytsr = await require("ytsr")(q)
 
 const mp4 = await downloadYouTube(ytsr.items[0].url)

 let size = mp4.mp3[1].size
 let bitrate = mp4.mp3[1].bitrate

 const start = await starTask(mp4.mp3[1].hash)
 
 const checkStatus = async(taskid) => {
 let progress = await taskStatus(start) 
 let text = `* [! YOUTUBE MP3 !]*`
 ...
 let aud = { audio: { url: download },  mimetype: 'audio/mpeg' }	
 return client.sendMessage(from, aud, { quoted : msg })
 } else {
